...
- **Phase 2 — Foundations.** Variable collections (3-tier or flat domain-based — both supported). Light/Dark modes (and multi-brand); responsive type via Desktop/Mobile modes. Text styles, effect styles. Sets codeSyntax.WEB on every variable. Semantic completeness check (bg / text / border / interactive / status / accent) — status colours are mandatory for app UIs. Skipped when foundations exist and pass health check.
